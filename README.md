...
1. Organise audio files into `real/` and `fake/` directories
2. Run `deepfake_audio_detection.ipynb` to extract features, train, and evaluate the model
3. (Optional) Run `SoundTransfer.ipynb` first if you want to regenerate the Kokoro-82M synthetic samples
4. Run `GraphGenerator.ipynb` to reproduce the result plots
